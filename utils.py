    ...
    # datasets in Geom-GCN
        ...
        with open(graph_node_features_and_labels_file_path) as graph_node_features_and_labels_file:
            ...
            for line in graph_node_features_and_labels_file:
                ...
                assert (len(line) == 3)
                assert (int(line[0]) not in graph_node_features_dict and int(line[0]) not in graph_labels_dict)
                graph_node_features_dict[int(line[0])] = np.array(line[1].split(','), dtype=np.float)
                graph_labels_dict[int(line[0])] = int(line[2])
...
def full_load_data(dataset_name, splits_file_path):
    if dataset_name in {'cora', 'citeseer', 'pubmed'}:
        g, nclass, features, labels, _, _, _ = preprocess_data(dataset_name,0.6)
    else:
        graph_adjacency_list_file_path = '../high_freq/{}/out1_graph_edges.txt'.format(dataset)
        graph_node_features_and_labels_file_path = '../high_freq/{}/out1_node_feature_label.txt'.format(dataset)
    ...
    splits_file_path = '.\BBAD'+splits_file_path
    ...
    features = torch.FloatTensor(features)
    labels = torch.LongTensor(labels)
    train = torch.BoolTensor(train)
    val = torch.BoolTensor(val)
    test = torch.BoolTensor(test)
